//
//  AdminOrderHistory.swift
//  FoodLocalAdmin
//
//  Created by Martin Parker on 05/02/2020.
...
class AdminOrderHistory: UIViewController, OrderCellDelegate {
    ...
    func setOrderListener(){
        ...
        listener = db.collection("order").whereField("isCompleted", isEqualTo: true).order(by: "timeStamp", descending:
        true).addSnapshotListener({ (snap, error) in
            
//            listener = db.collection("order").whereField("customerId", isEqualTo: Auth.auth().currentUser?.uid).addSnapshotListener({ (snap, error) in
...
extension AdminOrderHistory : UITableViewDelegate, UITableViewDataSource{
