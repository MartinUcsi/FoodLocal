...
//  FoodLocalRider
//
//  Created by Martin Parker on 16/02/2020.
...
class RiderHistoryVC: UIViewController {
    ...
    func setOrderListener(){
         ...
         guard let riderRef = Auth.auth().currentUser?.uid else {return}
          ...
          listener = db.collection("order").whereField("isCompleted", isEqualTo: true)
              .whereField("riderId", isEqualTo: riderRef)
...
extension RiderHistoryVC : UITableViewDelegate, UITableViewDataSource{
